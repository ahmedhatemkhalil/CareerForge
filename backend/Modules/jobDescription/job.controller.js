...
export const getJobDescriptionById = async (req, res) => {
    try {
        const { id } = req.params;
        const job = await JobDescription.findOne({ _id: id, createdBy: req.user.id || req.user.userId });
...
export const updateJobDescription = async (req, res) => {
    try {
        ...
        res.status(200).json({ success: true, message: "Updated successfully", data: job });
    } catch (error) {
        res.status(500).json({ success: false, message: error.message });
    }
};
...
export const deleteJobDescription = async (req, res) => {
    try {
        ...
        const job = await JobDescription.findOneAndDelete({ _id: id, createdBy: req.user.id || req.user.userId });
